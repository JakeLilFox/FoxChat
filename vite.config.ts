...
const elementCall = () => ({
  ...
  configureServer(server: {
    ...
    server.middlewares.use('/element-call', (req, res, next) => {
      ...
      if (!file.startsWith(elementCallDist) || !existsSync(file) || !statSync(file).isFile())
        return next()
      ...
      res.end(
        extension === 'js'
          ? injectElementCallMediaIdentity(content.toString('utf8')).source
          : content,
      )
    })
  },
  closeBundle() {
    ...
    if (!existsSync(elementCallDist)) return
    cpSync(elementCallDist, output, { recursive: true })
    ...
    while (pending.length) {
      ...
      for (const entry of readdirSync(directory, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) pending.push(path)
          ...
          const transformed = injectElementCallMediaIdentity(readFileSync(path, 'utf8'))
          replacements += transformed.replacements
          if (transformed.replacements) writeFileSync(path, transformed.source)
        }
      }
    }
    if (replacements !== 1)
      throw new Error(
        `Expected to tag one Element Call media hook, but tagged ${replacements}. Check the pinned embedded bundle.`,
      )
  },
})
